...
# 硬件警告與散熱
## 降電壓
![image](https://github.com/HYDeReUb/HY_main/blob/master/Picture%20Saved/under_volt.png)
```
...
```
## 溫度過高(80~85°C)
![image](https://github.com/HYDeReUb/HY_main/blob/master/Picture%20Saved/over_temperature_80_85.png)
```
...
```
## 溫度過高(85°C以上)
![image](https://github.com/HYDeReUb/HY_main/blob/master/Picture%20Saved/over_temperature_85.png)
```
...
## USB部份
``` 
1.若要提供SSD、HDD、光碟機這樣的高耗電外接裝置時，在足夠電力的情況下只能選一個來使用，若同時使用兩個可能會有不夠電的問題發生

2.若上述要使用2個以上時，要注意線材、電源及USB的電流負荷量，或是接上獨立電源的HUB來運作(HUB的電流負荷也得注意)
...
## 硬碟與記憶體部份
```
1.使用SD卡作主硬碟使用會有卡住的情況，屬正常現象。因為SD卡有多數(尤其是便宜貨)沒辦法負荷太多同時讀取與寫入的窘境，在加上使用虛擬記憶體的問題更考驗讀寫速度與延遲，也因此Chromium瀏覽器的網頁就
會有發生錯誤要重新讀取的狀況。我個人建議直接用SSD作主硬碟使用(Raspberry Pi已知問題的第2項)，不只減少chromium發生錯誤的狀況(這是以2G RAM版本作為參考)，也不在會有隨時卡住的情況，前提是不要
出現降電壓符號時使用

2.建議在執行格式化或移動檔案這樣的操作時，別執行其他程式來多工操作，避免使用量過高強關或重起導致資料毀損，也確保沒有顯示降電壓符號
```
## 其他部份
```
1.GPIO不要做熱插拔的動作，更不能短路3V3接腳，否則易造成損壞(Raspberry Pi 3 Model B+只要GND或5V去接觸3V3接腳，直接掛點!)

2.若出現降電壓符號或系統停擺時，確認電源跟線材的負荷量以及USB或GPIO的耗電量，做出適當的調整
```
